...
public:
    int numberOfArithmeticSlices(vector<int>& nums) {
        ...
        //ans是结果，d是公差
        int ans = 0, d = nums[0] - nums[1];
        int t = 0; //用于记录公差相同的时，子数组的个数
...
public:
    string replaceSpace(string s) {
        ...
        s.resize(len + 2*count); //重置字符串大小
        for(int i=len-1, j=s.size()-1; i<j ; i--){
            if(s[i] != ' ') s[j--] = s[i]; //不是空格移动字符
            //是空格就替代
            else{
                s[j--] = '0';
                s[j--] = '2';
...
class CQueue {
private:
    stack<int> s1; //用于在尾部添加元素
    stack<int> s2; //用于删除头部元素
public:
    CQueue() {}
