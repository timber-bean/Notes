...
* 关联容器
  * 无序容器：查询效率高：（散列表）unordered_set，multiset，map，multimap
...
| :-------: | :----: | :------: | :-------------------------------: |
| overload  |  相同  |   不同   |               相同                |
| override  |  相同  |   相同   |  不同（基类函数有virtual关键字）  |
| overwrite |  相同  |   不同   |  不同（基类函数无virtual关键字）  |
|           |        |   相同   | 不同（基类函数有无virtual无影响） |
...
* 自由存储区和堆的区别和联系
  ...
  * 联系：在C++中，是用malloc实现的new，物理上，自由存储区
...
* 堆和栈
  ...
  * 分配方式
    * 堆是动态分配，没有静态分配
    * 栈中既有静态分配又有动态分配，静态分配是编译器完成的，动态分配是由alloca函数分配，编译器自动释放，无需程序员实现
  * 空间大小
...
定义：程序员申请内存时，没有足够的内存供申请者使用。

原因：

* 内存中加载的数据量太大
* 集合类中有对对象的引用，使用后未清空，使得不能回收
